        ...
        {selected && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center">
            <div className="bg-white p-5 rounded-lg shadow-xl w-full max-w-md">
              ...
              <p className="mb-1"><strong>Proveedor:</strong> {selected.supplier}</p>
              <p className="mb-1"><strong>Monto:</strong> ${selected.amount.toLocaleString()}</p>
              <p className="mb-1"><strong>Categoría:</strong> {selected.category ?? (selected.tipo === "ingreso" ? DEFAULT_CATEGORY.ingreso : DEFAULT_CATEGORY.gasto)}</p>
              <p className="mb-3 text-sm text-slate-500"><strong>Fecha entrada:</strong> {formatDateYMD(selected.entryDate)}</p>
              <div className="flex gap-2 justify-end">
                <button onClick={handleModalEdit} className="bg-amber-500 text-white px-3 py-1.5 rounded-md text-sm">Editar</button>
                <button onClick={handleModalDelete} className="bg-red-500 text-white px-3 py-1.5 rounded-md text-sm">Eliminar</button>
                <button onClick={closeModal} className="bg-slate-100 px-3 py-1.5 rounded-md text-sm">Cerrar</button>
              </div>
            </div>
          </motion.div>
        )}
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center">
            ...
            <motion.div initial={{ scale: 0.98, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.98, opacity: 0 }} transition={{ duration: 0.14 }} className="relative bg-white p-4 rounded-lg shadow-lg z-10 w-full max-w-sm">
              ...
              <div className="flex justify-end gap-2">
                <button onClick={() => setToDelete(null)} className="px-3 py-1 rounded-md bg-slate-100 text-sm">Cancelar</button>
                <button onClick={confirmDelete} className="px-3 py-1 rounded-md bg-red-600 text-white text-sm">Eliminar</button>
              </div>
            </motion.div>
          </motion.div>
        )}
